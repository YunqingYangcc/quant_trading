...
> 版本：v0.6
> 更新日期：2026-06-20
> 状态：**Phase A-H+I+K 完成，Phase J 待开发**
---
...
| 赛道扩展 | 新增材料(material)、电力AI(ai-power)，共 6 条赛道 | P0 | ✅ |
...
**升级计划（Phase I）：**
- 接入 akshare 补充基本面数据：PE、PB、ROE、营收增速、北向资金 | P1 | ✅ 已完成
...
| 预测目标 | 未来 20 日赛道内相对排名（二分类：高于/低于中位数） | P0 | ✅ |
...
| 赛道 | 股票数 | Train Acc | Val Acc | Test Acc | 过拟合？ |
|:-----|:------:|:--------:|:------:|:-------:|:--------:|
| 半导体 | 17 | 55.8% | 54.0% | 52.8% | ✅ 否 |
| AI | 5 | ~54% | ~50% | ~50% | ✅ 否 |
| 机器人 | 5 | ~54% | ~50% | ~50% | ✅ 否 |
| 存储 | 5 | ~54% | ~50% | ~50% | ✅ 否 |
| 电力AI | 8 | 54.1% | 50.0% | 50.0% | ✅ 否 |
| 材料 | ~20 | ? | ? | ? | 待评估 |

**验证集准确率 50-54%，接近随机。模型信号较弱，但月频回测夏普可达 0.92。**
...
| **基准策略对比** | 添加简单动量策略作 baseline | 确认 AI 模型是否真的有增量 | 低 |
...
| 最大回撤 | < 25% |
| 分层收益 | 高分池持续跑赢低分池（单调） |
| 当前最佳 | 月频策略：夏普 0.92, 年化 19.8%, 最大回撤 -15.9% |
